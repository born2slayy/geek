// _ip050_DRIVER_HPP_
#ifndef _ip050_DRIVER_HPP_
#define _ip050_DRIVER_HPP_
...
#include <ip050_msgs/RobotMotor.h>

#define PI 3.141592
#define WHEEL_DIAMETER 0.13067 //0.130
#define WHEEL_WITH 0.444 //0.4
...
namespace aidl
{
	...
	public:
		// constructor"
		ip050BodyNode(ros::NodeHandle *nh) : x_(0), y_(0), th_(0), delta_x_(0), delta_y_(0), delta_th_(0), vx_(0), vy_(0), vth_(0),
		...
		void rpm_vel_callback(const ip050_msgs::RobotMotor &msg)
		{
		...
		{
			// [m/s]
			ip050_msgs::RobotMotor motor_msg;
		...
		void odom_compute(const ros::TimerEvent &e)
		{
			...
			delta_x_ = (1) * (1.018802626) * (cur_vx_ * cos(th_) - cur_vy_ * sin(th_)) * delta_t_; // ip050
			delta_y_ = (cur_vx_ * sin(th_) + cur_vy_ * cos(th_)) * delta_t_;
			delta_th_ = (0.97) * (0.90) * cur_vth_ * delta_t_;
